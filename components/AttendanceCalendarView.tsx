          ...
          <table className="w-full min-w-[980px] text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50">
                ...
                {weekDays.map(d => (
                  <th key={d.toISOString()} className="px-2 py-5 sm:py-6 text-center border-b border-slate-100 min-w-[108px]">
                    <div className="flex flex-col items-center">
                ...
                <tr key={emp.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="w-[190px] min-w-[190px] sm:w-[260px] sm:min-w-[260px] px-4 sm:px-6 py-4 sticky left-0 bg-white group-hover:bg-slate-50/50 transition-colors border-r border-slate-50">
                    <div className="flex items-center space-x-2 sm:space-x-3">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-500 text-sm shrink-0">
                        {emp.full_name.charAt(0)}
                      </div>
                      <div>
                        <p className="font-bold text-slate-700 text-xs sm:text-sm leading-tight">{emp.full_name}</p>
                        <p className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase leading-tight">{emp.sector_id ? (sectorMap[emp.sector_id] || emp.sector_id) : 'Sin Sector'}</p>
                      </div>
                    </div>
                  </td>
                  {weekDays.map(d => {
